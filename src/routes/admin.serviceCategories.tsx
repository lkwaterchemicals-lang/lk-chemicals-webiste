...
export const Route = createFileRoute("/admin/serviceCategories")({
  validateSearch: (s: Record<string, unknown>): S => ({
    q: typeof s.q === "string" && s.q ? s.q : undefined,
    new: s.new === "1" ? "1" : undefined,
  }),
  head: () => ({ meta: [{ title: "Service categories — LK Admin" }] }),
  component: ServiceCategoriesAdmin,
});

const def = moduleById("serviceCategories");

function ServiceCategoriesAdmin() {
  const { q, new: isNew } = Route.useSearch();
  const { data: services = [] } = useCol("services");
  ...
  const cols: Col<Row>[] = [
    ...
    },
    {
      ...
      label: "Services",
      hideBelow: "md",
      sortVal: (r) => services.filter((s) => s.serviceCategory === r.slug).length,
      csv: (r) => services.filter((s) => s.serviceCategory === r.slug).length,
      render: (r) => {
        const n = services.filter((s) => s.serviceCategory === r.slug).length;
        return (
          <span
