  ...
  head: () => ({
    meta: [
      { title: "Products — LK Chemicals Formulary" },
      {
        name: "description",
        content:
          "Full catalog of RO, boiler, cooling tower, chiller, descaling, ETP & STP and water treatment chemicals, resin & AHU cleaners, plus RO, DM, softener, STP and ETP plants from LK Chemicals.",
      },
...
function ProductsPage() {
  ...
  const { data: content } = useProductsContent();
          ...
          <div className="mx-auto max-w-7xl px-6 md:px-8">
            ...
            {categories.length === 0 && products.length === 0 && (
              <div className="mt-10 rounded-3xl glass-dark p-10 text-center">
                <h2 className="display-xl text-2xl text-white">The catalog is being set up.</h2>
                <p className="mt-2 text-white/60">
                  Products and categories appear here as soon as they're published from the
                  dashboard.
                </p>
              </div>
            )}
            {(categories.length > 0 || products.length > 0) && (
              <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                >
                  ...
                  <h2 className="display-xl text-2xl sm:text-3xl text-foreground mt-4">
                    All products
                  </h2>
      ...
      {browsing && (
        <section ref={gridRef} className="section-dark py-16 scroll-mt-24">
          <div className="mx-auto max-w-7xl px-6 md:px-8">
            ...
            <h2
              className="display-xl mt-8 grad-text"
              style={{ fontSize: "clamp(1.9rem, 5.5vw, 3.5rem)" }}
            >
              {q.trim() ? `Results for "${q.trim()}"` : activeCat ? activeCat.name : "All products"}
