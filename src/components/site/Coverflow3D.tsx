...
export function Coverflow3D({
  ...
  useEffect(() => {
      ...
      for (let i = 0; i < rail.children.length; i++) {
        ...
        if (!reduced) {
          card.style.transform =
            variant === "y"
              ? `perspective(1000px) rotateY(${(-t * 30).toFixed(2)}deg) scale(${(1 - a * 0.14).toFixed(3)}) translateZ(${(-72 * a).toFixed(1)}px) translateY(${(a * 10).toFixed(1)}px)`
          ...
          card.style.opacity = String(1 - a * 0.34);
          card.style.filter = a > 0.04 ? `brightness(${(1 - a * 0.18).toFixed(3)})` : "";
        }
      }
      setDot(closest);
    };
      ...
      : window.setInterval(() => {
          ...
          for (let i = 0; i < cards.length; i++) {
            ...
            if (
              ...
              Math.abs(
                (cards[cur] as HTMLElement).offsetLeft +
                  (cards[cur] as HTMLElement).offsetWidth / 2 -
                  mid,
              )
            )
              cur = i;
    ...
    return () => {
      ...
      for (const ev of ["pointerdown", "wheel", "touchstart"] as const)
        rail.removeEventListener(ev, markUser);
    };
    ...
    <div ref={wrapRef} className={className}>
      ...
      >
        {items.map((it, i) => (
          <div
            key={i}
            className={"snap-center shrink-0 transition-opacity duration-150 " + cardClass}
          >
            {it}
          </div>
        ))}
