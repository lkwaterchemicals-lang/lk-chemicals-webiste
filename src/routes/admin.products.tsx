...
export const Route = createFileRoute("/admin/products")({
  ...
  head: () => ({ meta: [{ title: "Products — LK Admin" }] }),
  component: ProductsAdmin,
});

const def = moduleById("products");
...
function ProductsAdmin() {
  ...
  const cols: Col<Row>[] = [
    ...
    },
    {
      id: "category",
      label: "Category",
      sortVal: (r) => String(r.category ?? ""),
      csv: (r) => String(r.category ?? ""),
      render: (r) => <Badge tone="neutral">{String(r.category ?? "—")}</Badge>,
    },
    {
      id: "features",
      label: "Features",
      hideBelow: "lg",
      sortVal: (r) => (Array.isArray(r.features) ? r.features.length : 0),
      csv: (r) => (Array.isArray(r.features) ? (r.features as string[]).join(" | ") : ""),
      render: (r) => (
