...
function applyTheme(t: Theme) {
  ...
  try { localStorage.setItem("lk-theme", t); } catch {}
}
...
export function ThemeToggle({ className = "" }: { className?: string }) {
  ...
  const toggle = useCallback((e: React.MouseEvent) => {
    ...
    const start = (document as Document & { startViewTransition?: (cb: () => void) => { finished: Promise<void> } }).startViewTransition?.bind(document);
    if (start) {
  ...
  return (
    ...
    >
      <Sun className={"h-4 w-4 absolute transition-all duration-500 " + (theme === "light" ? "rotate-0 opacity-100 scale-100" : "-rotate-90 opacity-0 scale-50")} />
      <Moon className={"h-4 w-4 absolute transition-all duration-500 " + (theme === "dark" ? "rotate-0 opacity-100 scale-100" : "rotate-90 opacity-0 scale-50")} />
    </button>
  );
}