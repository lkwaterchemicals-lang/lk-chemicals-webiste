...
import { ArrowRight, Droplets, Loader2, Lock, Mail, ShieldCheck, Sparkles, Zap } from "lucide-react";
...
function Login() {
  ...
  return (
    <div className="grid min-h-screen lg:grid-cols-[1.1fr_1fr]">
      ...
      >
        <div className="pointer-events-none absolute -right-32 -top-32 h-96 w-96 rounded-full opacity-25 blur-3xl" style={{ background: "#38bdf8" }} />
        <div className="pointer-events-none absolute -left-24 bottom-0 h-80 w-80 rounded-full opacity-15 blur-3xl" style={{ background: "#3b82f6" }} />
        <div className="relative flex items-center gap-3">
          <img src={logoUrl} alt="" className="h-10 w-10 object-contain" />
          <div>
            <div className="text-sm font-bold text-white" style={{ fontFamily: "var(--font-display)" }}>LK Chemicals</div>
            <div className="text-[10px] font-semibold uppercase tracking-[0.16em] text-white/50">Admin Console 3.0</div>
          </div>
        </div>
        <div className="relative">
          <h1 className="max-w-md text-4xl leading-tight !text-white">Run the whole website from one console.</h1>
          <p className="mt-4 max-w-sm text-sm text-white/60">
            Catalog, services, media, enquiries and site settings — every change goes live on lkchemicals.in the moment you save.
          </p>
          <div className="mt-8 grid max-w-sm grid-cols-3 gap-3">
            {[
              ...
              <div key={f.label} className="rounded-2xl border border-white/10 bg-white/5 p-3 text-center">
      ...
      <div className="grid place-items-center px-5 py-12">
        <form onSubmit={submit} className="w-full max-w-sm a-rise">
          ...
          <label className="mt-8 block">
            <span className="text-xs font-semibold" style={{ color: "var(--a-text2)" }}>Email</span>
            <div className="relative mt-1.5">
              <Mail className="pointer-events-none absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2" style={{ color: "var(--a-text3)" }} />
              <input
          ...
          <label className="mt-4 block">
            <span className="text-xs font-semibold" style={{ color: "var(--a-text2)" }}>Password</span>
            <div className="relative mt-1.5">
              <Lock className="pointer-events-none absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2" style={{ color: "var(--a-text3)" }} />
              <input
                className="a-input !pl-9"
                type="password"
          ...
          {error && <p className="mt-3 text-xs font-medium" style={{ color: "var(--a-danger)" }}>{error}</p>}
          {notice && <p className="mt-3 text-xs font-medium" style={{ color: "var(--a-ok)" }}>{notice}</p>}
          ...
          <button type="button" onClick={forgot} className="mt-4 w-full text-center text-xs underline-offset-2 hover:underline" style={{ color: "var(--a-text3)" }}>
            Forgot password?
          </button>
        </form>
      </div>
